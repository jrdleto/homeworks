...
function check1(){
  let mizani = document.getElementsByClassName('average');
  for(let i = 1; i < mizani.length; i++){
    if(mizani[i].textContent >= 2){
      mizani[i].style.backgroundColor = "darkblue";
    }
    else{
      mizani[i].style.backgroundColor = "gold";
    }
  }
}
...
function event1(event){
    ...
    else if(event.target.textContent >=2 && event.target.textContent <= 4){
...
//all the values, for each average lane.
    ...
    for(let i = 1; i < 8; i++){ // i equals 1 cause,first content is 'Average'
        average[i].appendChild(document.createTextNode((parseFloat(array[i-1])/content.length).toFixed(1))); 
